...
${BOLD}SECURITY:${NC}
  ...
  - Generated passphrases are cryptographically random (12 chars, all types)
...
gen_passphrase() {
  ...
  # Generate 12-character passphrase with all 4 character types distributed throughout
  # Exclude ambiguous: l, 1, I, O, 0, o for better readability
  local symbols="!@#$%^&*"
  ...
  local all_chars="${uppers}${lowers}${digits}${symbols}"
  
  # Generate 12 random characters from all sets
  local pass=""
  for i in {1..12}; do
    local random_byte=$(openssl rand -hex 1)
    local index=$((0x$random_byte % ${#all_chars}))
    pass="${pass}${all_chars:$index:1}"
  done
  
  # Verify all 4 character types are present (extremely rare to miss, but guarantee it)
  if [[ ! "$pass" =~ [A-Z] ]] || [[ ! "$pass" =~ [a-z] ]] || [[ ! "$pass" =~ [0-9] ]] || [[ ! "$pass" =~ [!@#\$%\^\&\*] ]]; then
    # Ensure at least one of each type by replacing first 4 chars
    ...
    # Replace first 4 positions with guaranteed types, keep rest random
    pass="${sym_char}${dig_char}${upp_char}${low_char}${pass:4}"
  fi
...
get_passphrase() {
  ...
  while [ $validation_attempts -lt $max_validation_attempts ]; do
    if [ $validation_attempts -eq 0 ]; then
      ...
      echo "" >&2
      echo "${CYAN}Choose a passphrase for dashboard access${NC}" >&2
      echo "This will encrypt your monitoring data" >&2
      echo "" >&2
      echo "${BOLD}Requirements:${NC} 12+ chars OR 3+ words, with 3 of 4 types (upper/lower/digit/symbol)" >&2
      echo "${BOLD}Easy option:${NC} Press Enter to auto-generate a strong passphrase" >&2
      echo "" >&2
    else
      # Retry attempts - show condensed prompt
      echo "" >&2
      echo "${CYAN}Try again${NC} (attempt $(($validation_attempts + 1)) of $max_validation_attempts)" >&2
      echo "${BOLD}Tip:${NC} Press Enter to auto-generate instead" >&2
      echo "" >&2
    fi
    ...
    if validate_passphrase "$pass" true; then
      ...
      while true; do
        ...
        echo "${RED}✗${NC} Passphrases do not match (attempt $retries/$MAX_MISMATCH_RETRIES)" >&2
        echo "" >&2
        ...
        if ! validate_passphrase "$pass" false; then
          echo "${RED}✗${NC} Passphrase still doesn't meet requirements" >&2
...
ENV_EOF
  ...
  if [[ "$save_pass" =~ ^[Yy]$ ]]; then
    # Use %q to properly escape ALL special characters for shell safety
    printf 'DASH_PASSPHRASE=%q\n' "$passphrase" >> .env
